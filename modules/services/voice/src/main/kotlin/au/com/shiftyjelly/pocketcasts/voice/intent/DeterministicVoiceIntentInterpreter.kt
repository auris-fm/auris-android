...
class DeterministicVoiceIntentInterpreter @javax.inject.Inject constructor() : VoiceIntentInterpreter {
    override suspend fun interpret(result: VoiceRecognitionResult): VoicePlaybackIntent? {
        ...
        val text = result.transcript.trim().lowercase()
        if (text.length < 2) return null

        return when {
            text == "pause" || text == "stop" -> VoicePlaybackIntent.Pause
            text == "resume" || text == "play" || text == "resume playback" -> VoicePlaybackIntent.Resume
            ...
            text.contains("skip back") || text.contains("go back") || text == "rewind" -> {
                VoicePlaybackIntent.SeekRelative(-parseSeconds(text, 10) * 1000)
            }
            text == "skip forward" || text.contains("skip ahead") || text == "fast forward" -> {
                VoicePlaybackIntent.SeekRelative(parseSeconds(text, 30) * 1000)
            }
            text == "skip" || text == "forward" -> {
                VoicePlaybackIntent.SeekRelative(parseSeconds(text, 30) * 1000)
            }
            else -> null
        }
    ...
    private fun parseSeconds(text: String, defaultSeconds: Int): Int {
        return when {
            text.contains("one minute") || text.contains("60 seconds") -> 60
